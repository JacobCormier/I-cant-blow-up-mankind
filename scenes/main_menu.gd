...
func start_tween_animation():
	...
	tween.tween_property(lid, "rotation_degrees:z", 26.5, TIME_SCALE * 2)
	...
	tween.set_ease(Tween.EASE_IN)
	tween.parallel().tween_property(player, "position:y", 500, TIME_SCALE * 8)
	...
	tween.tween_interval(0)
	tween.parallel().tween_property(camera_3d, "position:x", 0, TIME_SCALE * 8)
	...
	tween.parallel().tween_property(camera_3d, "rotation_degrees:x", -13.7, TIME_SCALE * 8)
